...
1. **完整 http/https URL**（不限域名，不限平台）
   ...
   - 覆盖 `v.douyin.com`、B站 `bilibili.com`、YouTube `youtube.com` 等任意链接
   - 提取后去除尾部 `/`
2. **短编码模式**（`xxx:/`，排除常见协议名）
...
{
  "texts": [
    "0.02 复制打开视频，看看【胡说老木丶的作品】DeepSeek最强提示词... https://v.douyin.com/YY4lbvaPHsU/ K@W.md OKJ:/ :2pm 11/01",
